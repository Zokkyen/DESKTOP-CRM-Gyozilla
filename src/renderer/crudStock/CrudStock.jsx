...
const CrudStock = () => {
    ...
    const leftToolbarTemplate = () => {
        return (
            <div className="flex flex-wrap gap-2">
                ...
                <Button label="Supprimer" icon="pi pi-trash" severity="danger" onClick={confirmDeleteSelected} disabled={!selectedStocks || !selectedStocks.length} />
            </div>
        );
    };
    ...
    return <Typography variant='BUTTON TEXT'>{rowData.ingredients.name}</Typography>;
    };
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <Button icon="pi pi-pencil" style={{color: '#212830', marginRight: '6px'}} rounded outlined onClick={() => editStock(rowData)} />
                <Button icon="pi pi-trash" rounded outlined severity="danger" onClick={() => confirmDeleteStock(rowData)} />
            </React.Fragment>
        );
    };
    ...
    return (
        <div>
            ...
            <div className="card">
                ...
                <DataTable scrollable scrollHeight='50vh' ref={dt} value={stocks} selection={selectedStocks} onSelectionChange={(e) => setSelectedStocks(e.value)}
                    ...
                    <Column field="quantity" header="Quantité" body={quantityBodyTemplate} sortable style={{ minWidth: '8rem' }}></Column>
                    <Column field="stockStatus" header="Status" body={statusBodyTemplate} sortable style={{ minWidth: '12rem' }}></Column>
        ...
        {/* Modal details et modif */}
            ...
            <Dialog visible={deleteStockDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Valider" modal onHide={hideDeleteStockDialog}>
                <div className="confirmation-content">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {stock && (
                        <span> Êtes-vous sûr de vouloir supprimer le <b>stock</b>?</span>
                    )}
                </div>
                <Box sx={{display: 'flex', justifyContent: 'center', marginTop: '20px'}}>
                <Button style={{marginRight: '10px', color: '#4f7170', border: '1px solid #4f7170'}} label="Non" icon="pi pi-times" outlined onClick={hideDeleteStockDialog} />
                <Button label="Oui" icon="pi pi-check" severity="danger" onClick={() => deleteStock(stock.id)}/>
                </Box>
            </Dialog>
            
            {/* Modal delete selection ingredient */}
            <Dialog visible={deleteStocksDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Valider" modal onHide={hideDeleteStocksDialog}>
                <div className="confirmation-content">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {stock && <span> Êtes-vous sûr de vouloir supprimer plusieurs lignes du stock ?</span>}
                </div>
                <Box sx={{display: 'flex', justifyContent: 'center', marginTop: '20px'}}>
                <Button style={{marginRight: '10px', color: '#4f7170', border: '1px solid #4f7170'}} label="Non" icon="pi pi-times" outlined onClick={hideDeleteStocksDialog} />
                <Button label="Oui" icon="pi pi-check" severity="danger" onClick={deleteSelectedStocks} />
                </Box>
            </Dialog>
        </div>
    );
